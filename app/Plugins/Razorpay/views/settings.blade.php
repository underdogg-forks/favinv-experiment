...
@section('content-header')
    ...
    <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item active">{{ trans('message.razorpay') }}</li>
        </ol>
    </div><!-- /.col -->
@stop
        ...
        <div class="row">
            ...
            <table class="table table-condensed">
                    <tr>
                        ...
                        {!! html()->label( trans('message.rzp_key'), 'rzp_key')->class('required') !!}
                ...
                <tr>
                        <td>
                          <!-- last name -->
                            {!! html()->label( trans('message.rzp_secret'), 'rzp_secret')->class('required') !!}
                            <div class="form-group col-lg-5 pl-0">
                ...
                <tr>
                          <td>
                          <!-- last name -->
                              {!! html()->label( trans('message.api_layer_access_key'), 'apilayer_key')->class('required') !!}
                              <div class="form-group col-lg-5 pl-0">
               ...
               <br>
                <button type="submit" class="btn btn-primary btn-sm" id="key_update"><i class="fa fa-sync-alt"></i>&nbsp;{{ trans('message.update') }}</button>
              ...
              if (!isValid) {
                  ...
                  $.ajax ({
                      ...
                      success: function (data) {
                          ...
                          $('#key_update').html("<i class='fas fa-circle-notch fa-spin'></i>  {{ trans('message.update') }}");

                          $('#alertMessage').show();
                          var result =  '<div class="alert alert-success alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><strong><i class="fa fa-check"></i> {{ trans('message.success') }}! </strong>'+data.message.message+'.</div>';
                          $('#alertMessage').html(result+ ".");
                          $("#key_update").html("<i class='fa fa-sync-alt'>&nbsp;</i>{{ trans('message.update') }}");
                          ...
                          $('#key_update').html("<i class='fas fa-circle-notch fa-spin'></i>  {{ trans('message.update') }}");
                          $('#errorMessage').show();
                          var result =  '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><strong><i class="fa fa-ban"></i> {{ trans('message.failed') }}! </strong>'+data.responseJSON.message+'.</div>';
...
<script type="text/javascript">
             ...
             $('#b_currency_update').on('click',function(){
                 $('#b_currency_update').html("<i class='fa fa-circle-o-notch fa-spin fa-1x fa-fw'></i>{{ trans('message.please_wait') }}");
              ...
              data: {
               ...
               success: function (data) {
                    ...
                    $('#alertMessage').html(result+ ".");
                    $("#b_currency_update").html("<i class='fa fa-floppy-o'>&nbsp;&nbsp;</i>{{ trans('message.save') }}");
                      setInterval(function(){ 
                        $('#alertMessage').slideUp(3000); 
                    }, 1000);
                  },
