    ...
    public function addProduct(int $id, $domain = null)
    {
        try {
            ...
            if ($plan) { //If Plan For a Product exists
                ...
                throw new \Exception(trans('message.product_add_cart_no_plan'));
            }
    ...
    private function checkProductsHaveSimilarCurrency($currency)
    {
        ...
        foreach ($carts as $cart) {
            if ($cart->attributes['currency'] != $currency) {
                throw new \Exception(trans('message.similar_currency_required'));
            }
        }
    }
        ...
        try {
            ...
            foreach ($cartCollection as $item) {
                ...
                if ($unpaidInvoice) {
                    ...
                    ->with('warning', trans('message.unpaid_invoice_warning'));
                }
            }
            ...
            if ($product->status == 1) {
                ...
                if (! $admin) {
                    if (\Session::get('toggleState') == 'yearly' || \Session::get('toggleState') == null) {
                        ...
                        $daysQuery = PlanPrice::where('plan_id', $id)->where('currency', $currency['currency'])->where('country_id', $countryids)->firstOr(function () use ($currency, $id) {
                            return PlanPrice::where('plan_id', $id)->where('currency', $currency['currency'])->where('country_id', 0)->first();
                        });
